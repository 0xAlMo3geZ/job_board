...
<template>
    <div class="grid md:grid-cols-4 gap-3 py-10 px-6">
        <div class="md:col-span-1 px-6 py-6 bg-cyan-700 rounded-xl">
            <div class="flex space-x-4">
                <input v-model="query" type="search" placeholder="Find a job..." class="w-full px-6 py-4 rounded-xl">

                <button class="px-6 py-4 bg-cyan-900 text-white rounded-xl" v-on:click="performSearch">
            ...
            <div class="mt-6 space-y-4">
                <p v-for="category in jobCategories" v-bind:key="category.id" v-on:click="toggleCategory(category.id)"
                    class="py-4 px-6 text-white rounded-xl"
                    v-bind:class="{ 'bg-cyan-900': selectedCategoriesRef.includes(category.id) }">
                    {{ category.title }}
                </p>
            </div>
        </div>
